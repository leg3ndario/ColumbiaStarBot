...
def upload_leads():
    ...
    with open(CSV_FILE, mode='r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            # Basic validation (must start with a digit and not be a giant paragraph)
            if not raw_addr or not raw_addr[0].isdigit() or len(raw_addr) > 100:
                continue
            ...
            try:
                ...
                # Check if it was created or if it already exists
                ...
                if response.status_code in [200, 201] and lead_id:
                    ...
                    # The API response for 'already added' often includes the existing Lead ID
                    ...
                    if existing_id:
                        ...
                        if update_res.status_code == 200:
                            ...
                            print(f"⚠️ Found but couldn't update: {addr}")
                    else:
                        print(f"➖ Skipped: {addr} (Already in DM, No ID found to update)")
